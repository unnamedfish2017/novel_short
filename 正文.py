...
for scene in scenes[:4]:
    ...
    result=chat('%s\n%s\n请将上述内容展开为一段大约3000字的小说描写，需要符合人物性格设定，根据需要加入一些语言外貌心理描写，语言生动,注意不要出现逻辑错误,注意不要写其他分镜的内容，'%scene, history)
...
def chat_kimi_rewrite(query, history):
    # 将用户的问题添加到对话历史中
    history.append(
    {"role": "system", "content": "你是一名优秀的网络小说作家,你将收到一段小说文字\
        ,请帮我修改为10000字左右的小说,如果不同分镜之间有重复的内容,请帮忙修改,去除重复内容;\
...
history = [system_message]
result=chat_kimi_rewrite(正文tmp, history)
